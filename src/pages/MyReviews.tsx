    ...
    <div className="flex w-full flex-col gap-[32px]">
      <div className="flex h-[56px] items-center gap-[8px] rounded-[16px] p-[8px]" style={{ backgroundColor: 'var(--color-neutral-100)', width: 'fit-content' }}>
        {TABS.map((tab) => {
          ...
          return (
            <Link key={tab.path} to={tab.path} className="flex h-[40px] w-[175px] items-center justify-center rounded-[12px] px-[12px] py-[8px] transition-all" style={{ backgroundColor: isActive ? 'white' : 'transparent', boxShadow: isActive ? '0px 0px 20px 0px rgba(203,202,202,0.25)' : 'none', fontFamily: 'var(--font-family-quicksand)', fontSize: 'var(--font-size-text-md)', fontWeight: isActive ? 700 : 500, color: isActive ? 'var(--color-neutral-950)' : 'var(--color-neutral-600)', textDecoration: 'none' }}>
              {tab.label}
            </Link>
          );
        })}
